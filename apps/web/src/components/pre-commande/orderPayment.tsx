...
export function OrderPayment({ total, onBack }: OrderPaymentProps) {
    ...
    const [formData, setFormData] = useState({
        nom: '', prenom: '', email: '', phone: '', transactionRef: ''
    });

    const deposit = total * 0.5;
    const clientRef = `IRK-${Math.floor(1000 + Math.random() * 9000)}-2026`;
    ...
    if (isSuccess) {
        return (
            <div className="max-w-md mx-auto p-8 bg-white rounded-[40px] shadow-2xl border border-slate-100 text-center animate-in zoom-in duration-500">
                ...
                <h2 className="text-2xl font-black text-slate-900 mb-2">Merci pour votre achat !</h2>
                <p className="text-slate-500 text-xs mb-8">Un mail de confirmation est envoyé à <span className="font-bold text-slate-900">{formData.email}</span></p>

                <div className="bg-slate-50 rounded-3xl p-6 mb-8 border border-dashed border-slate-200">
                    <p className="text-[9px] font-black uppercase text-slate-400 tracking-[0.2em] mb-1">Référence Client</p>
            ...
            <div className="bg-slate-950 text-white p-7 rounded-[32px] shadow-2xl relative overflow-hidden">
                <div className="relative z-10 space-y-4">
                    <div className="flex justify-between items-start">
                        <div>
                            <p className="text-[10px] font-black uppercase tracking-[0.2em] text-green-400 mb-1">Acompte à régler (50%)</p>
                        ...
                        <div className="bg-white/10 backdrop-blur-md px-3 py-1.5 rounded-xl border border-white/10 text-right">
                            <p className="text-[8px] font-bold uppercase tracking-tighter opacity-60">Total achat</p>
                    ...
                    <div className="flex items-center gap-1.5 py-1 px-2.5 bg-green-500/20 rounded-lg border border-green-500/30 w-fit">
                        ...
                        <span className="text-[8px] font-black uppercase tracking-wider text-green-400">Paiement sécurisé</span>
                    </div>
                </div>
            </div>
            ...
            {step === 1 ? (
                        ...
                        <div className="space-y-4 animate-in slide-in-from-top-4 duration-500">
                            <div className="bg-white border border-slate-100 rounded-[28px] p-5 space-y-4 shadow-sm">
                                <div className="space-y-1.5">
                                    <span className="text-[9px] font-black text-slate-400 uppercase tracking-widest ml-1">Compte {currentOp.alt}</span>
                                ...
                                <div className="grid grid-cols-2 gap-3">
                                    <div className="flex flex-col items-center justify-center p-3 bg-slate-900 rounded-xl text-white">
                                        ...
                                        <span className="text-[7px] uppercase font-bold text-slate-500">Code USSD</span>
                                    ...
                                    <div className="flex flex-col items-center justify-center p-2 bg-slate-50 border border-dashed border-slate-200 rounded-xl">
                                        ...
                                        <span className="text-[7px] font-bold uppercase text-slate-400 text-center leading-none">Scanner QR</span>
                                    </div>
                                </div>
                            </div>
                            <button onClick={() => setStep(2)} className="w-full bg-green-600 text-white py-4 rounded-2xl font-black uppercase tracking-[0.15em] text-[10px] hover:bg-green-700 transition-all shadow-xl shadow-green-50 flex items-center justify-center gap-2">
                                J'ai effectué le transfert <CheckCircle2 size={14} />
                            </button>
                        </div>
                    )}
                </div>
                ...
                <div className="space-y-5 animate-in slide-in-from-right-4 duration-500">
                    <div className="flex items-center gap-3 px-1">
                        ...
                        <h3 className="font-black text-slate-900 uppercase text-[9px] tracking-[0.2em]">Validation & Coordonnées</h3>
                    </div>

                    <div className="grid gap-3">
                        <div className="grid grid-cols-2 gap-3">
                            <input type="text" placeholder="Nom" value={formData.nom} onChange={(e) => setFormData({ ...formData, nom: e.target.value })} className="w-full bg-slate-50 border border-slate-100 rounded-xl py-4 px-5 text-xs font-bold focus:border-green-500 focus:bg-white outline-none transition-all" />
                            <input type="text" placeholder="Prénom" value={formData.prenom} onChange={(e) => setFormData({ ...formData, prenom: e.target.value })} className="w-full bg-slate-50 border border-slate-100 rounded-xl py-4 px-5 text-xs font-bold focus:border-green-500 focus:bg-white outline-none transition-all" />
                        </div>
                        <input type="email" placeholder="Email (pour le reçu PDF)" value={formData.email} onChange={(e) => setFormData({ ...formData, email: e.target.value })} className="w-full bg-slate-50 border border-slate-100 rounded-xl py-4 px-5 text-xs font-bold focus:border-green-500 focus:bg-white outline-none transition-all" />

                        <div className="mt-2 p-5 bg-green-50/30 border-2 border-dashed border-green-200 rounded-[28px]">
                            <label className="block text-[8px] font-black uppercase text-green-700 mb-2 tracking-[0.3em] text-center">ID Transaction (Reçu par SMS)</label>
                            <input type="text" placeholder="EX: 12345678" value={formData.transactionRef} onChange={(e) => setFormData({ ...formData, transactionRef: e.target.value })} className="w-full bg-white border-2 border-green-200 rounded-xl py-4 px-4 text-2xl font-mono font-black text-green-700 outline-none text-center uppercase tracking-widest shadow-inner focus:border-green-500" />
                        </div>
                    </div>
                    ...
                    <button onClick={handleFinalValidation} disabled={isSubmitting} className="w-full bg-slate-950 text-white py-5 rounded-[24px] font-black uppercase tracking-[0.2em] text-[10px] hover:bg-green-600 disabled:bg-slate-100 transition-all flex items-center justify-center gap-3 shadow-xl">
                        {isSubmitting ? <Loader2 className="animate-spin" size={16} /> : <>Confirmer & Recevoir le reçu <Mail size={16} /></>}
                    </button>
                </div>
            )}
        </div>
